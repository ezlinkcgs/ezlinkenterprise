...
int EzAgentTMSDriver :: initAgentTMSDriver( const char *TaskName )
{
	char logpath[ LOG_PATH_SIZE + 1 ], BitMapFile[ TMS_BITMAP_FILE_SIZE + 1 ],MerchId[ 33 ];

	memset( logpath, 0, sizeof( logpath ) );
	sprintf( logpath, "%s/%s/%s", getenv( "EZDIR_HOME" ), "log", "ezagenttmsdriver.log" );
	...
	trace( "^^^^^^^^^^^EzAgentTMSDriver Started Running^^^^^^^^^^^^^" );

	if( InitEzTaskUtil( this ) < 0 )
	{
		trace( "Error in Initializing EzTaskUtil");
		trace( "^^^^^^^^^^^EzAgentTMSDriver Ended Abnormally^^^^^^^^^^^^^");
		closeDebug();
		exit( 0 );
	}
	...
	if( CreateChannel( "EzAgentTMSDriver", TaskName ) < 0 )
	{
...
*/
	...
	{
		trace( "Error in Initializing AgentMMSUtil..." );
		trace( "^^^^^^^^^^^EzAgentTMSDriver Ended Abnormally^^^^^^^^^^^^^");
		closeDebug();
		TaskExit();
	}

	//if( ezmmsdbutil.initMMSDBUtil( this, this, &dbutil, &ezdatetimeutil,&amtutil,&tkutil, this ) < 0 )
	if( ezmmsdbutil.initEzMMSDBUtil( this, this, &ezdatetimeutil,&amtutil,&tkutil ) < 0 )
	{
...
int EzAgentTMSDriver :: logTMSTxnInfo( char LogFlag, const char *TMSTxnRefNum, struct EzTxnMsg *tmsmsg_ptr )
{
...
void EzAgentTMSDriver :: process500( struct EzTxnMsg tmsmsg )
{
...
void EzAgentTMSDriver :: process320( struct EzTxnMsg *tmsmsg_ptr )
{
...
void EzAgentTMSDriver :: populateOldTerminalRequest( struct EzTxnMsg *tmsmsg, struct EzTxnMsg *oldtmsmsg )
{
...
int EzAgentTMSDriver :: logTerminalStatus( struct EzTxnMsg *tmsmsg, char StatusId )
{
	int retval;
...
void EzAgentTMSDriver :: processEchoRequest( struct EzTxnMsg *tmsmsg )
{
...
void EzAgentTMSDriver :: processKeyRequest( struct EzTxnMsg *tmsmsg )
{
	int retval;
	...
	sprintf( TempDate, "%06d", tmsmsg->LocalDate );
	...
	//{
		if( strlen( tmsmsg->RsvFld2 ) >= 6 )
		{
			...
			if( ezmmsdbutil.insertTermUserLog( tmsmsg->AcquirerInst, tmsmsg->MachineId, tmsmsg->AcceptorId, tmsmsg->TerminalId, CatCode, UserName, TempDate ) < 0 )
			{
				tmsmsg->RespCode = 5;
				tmsmsg->MsgType += 10;
				writeToPOS( GetLastSenderChannelName(), tmsmsg );
...
bool EzAgentTMSDriver :: isTxnAllowed( const char *I_InstId, const char *I_TerminalType, const char *I_TxnCode, const char *I_CHN )
{
	int retval;
	char BinList[ 129 ];
	char TempChn[ 20 ];
...
bool EzAgentTMSDriver :: isTxnAllowedonProduct( const char *I_InstId, const char *I_TerminalType, const char *I_TxnCode, const char *I_CHN )
{
	int retval;
	char BinList[ 129 ];
	char TempChn[ 20 ];
...
bool EzAgentTMSDriver :: isValidTerminal( struct EzTxnMsg *tmsmsg_ptr )
{
	int retval;
...
*/
	...
	trace ("merchantprofile_rec.MerchantAcctNo [%s]", merchantprofile_rec.MerchantAcctNo );
	strcpy( tmsmsg_ptr->RsvFld4, merchantprofile_rec.MerchantAcctNo );
	...
	//Added to handle Multiple merchant currency for Agent or merchant POS settlement acct
	if( strlen( tmsmsg_ptr->RsvFld4 ) > 15 )  
	{
		memset( Str1, 0, sizeof( Str1 ) );
        	tkutil.getToken( tmsmsg_ptr->RsvFld4, Str1, '|', 1 );
        	memset( Str2, 0, sizeof( Str2 ) );
        	tkutil.getToken( tmsmsg_ptr->RsvFld4, Str2, '|', 2 );
        	trace( "Str1[%s] Str2[%s]", Str1, Str2 );
	 	
		if ( tmsmsg_ptr->AcqCurrencyCode == 971 && ( !strncmp( Str1, "971", 3 ) || !strncmp( Str1, "840", 3 ) ) )
	 	{
			memset( tmsmsg_ptr->RsvFld4, 0, sizeof( tmsmsg_ptr->RsvFld4 ) );
			strncpy( tmsmsg_ptr->RsvFld4, Str1 + 3, 15 );	
			trace ("AcqCurrencyCode [%d] ,RsvFld4 [%s]", tmsmsg_ptr->AcqCurrencyCode, tmsmsg_ptr->RsvFld4 );
		}
		else if ( tmsmsg_ptr->AcqCurrencyCode == 840 &&  ( !strncmp( Str1, "971", 3 ) || !strncmp( Str1, "840", 3 ) ) )
		{
			memset( tmsmsg_ptr->RsvFld4, 0, sizeof( tmsmsg_ptr->RsvFld4 ) );
                        strncpy( tmsmsg_ptr->RsvFld4, Str2 + 3, 15 );
			trace ("AcqCurrencyCode [%d] ,RsvFld4 [%s]", tmsmsg_ptr->AcqCurrencyCode, tmsmsg_ptr->RsvFld4 );
		}
	}
...
int EzAgentTMSDriver :: writeToInternalChannel( const char *ChannelName, struct EzTxnMsg *tmsmsg )
{
...
void EzAgentTMSDriver :: writeToExternalChannel( const char *ChannelName, char *ResponseBuffer, int MsgSize )
{
...
void EzAgentTMSDriver :: writeToPOS( const char *ChannelName, struct EzTxnMsg *tmsmsg )
{
	int ChannelKeyId, ChannelQid, MsgSize;

	//For reversal
	/*if( tmsmsg->MsgType == 210 && ( tmsmsg->TxnCode / 10000 == 40 || tmsmsg->TxnCode / 10000 == 71 || tmsmsg->TxnCode / 10000 == 0 || tmsmsg->TxnCode / 10000 == 68  || tmsmsg->TxnCode / 10000 == 57 ) )
	{
...
void EzAgentTMSDriver :: populateNewTerminalRequest( struct EzTxnMsg *tmsmsg )
{
	char temp[ 50 ];
...
void EzAgentTMSDriver :: serviceRequest()
{
	...
	while( 1 )
	{
		...
		*/
		...
		if( tmsmsg.TxnCode/10000 == 68 && tmsmsg.MsgType == 210 && strlen( tmsmsg.RsvFld1 ) )
		{
			memset(tmsmsg.private_use.EchoBack, 0, sizeof(tmsmsg.private_use.EchoBack) );
			sprintf ((char *) tmsmsg.private_use.EchoBack, "%s", tmsmsg.RsvFld1 );
			trace( "tmsmsg.private_use.EchoBack[ %s ]", tmsmsg.private_use.EchoBack );
		}
		...
		if( isInBoundTxnMsg() && tmsmsg.MsgType != 400 && tmsmsg.MsgType != 420 )
		{
			...
			if(  isInBoundNewTxnRequest() || isInBoundNewFinTxnRequest( )  )
			{
				...
				//Added by Hari 11JAN21 for valid null pinblock
				memset( PosCode, 0, sizeof( PosCode ) );
			        sprintf( PosCode, "%03d", tmsmsg.PosEntryCode );
			        trace("Pos Code DE 22[%s]",PosCode);
				//if( ( ( !strncmp( PosCode, "021", 3 ) ) || ( !strncmp( PosCode, "051", 3 ) ) ) &&  tmsmsg.PinVerFlg == 'N' ) 
				if( ( ( !strncmp( PosCode, "021", 3 ) ) || ( !strncmp( PosCode, "051", 3 ) ) ) &&  tmsmsg.PinVerFlg == 'N' && tmsmsg.TxnCode/10000 != 56 ) 
        			{
				...
				if ( !strncmp( tmsmsg.chn, "601540", 6 ) && ( tmsmsg.AcqCurrencyCode == 840 ) )
        			{
	                                trace( "PREPAID TXN USD CURRENCY NOT allowed." );
					tmsmsg.MsgType += 10;
                			tmsmsg.RespCode = EZTMS_RC_TransactionNotAllowed;
			        	writeToPOS( GetLastSenderChannelName(), &tmsmsg );
                                        continue;
				...
				/*
				// Added for Limit check for Agency POS MOHAN
				if( tmsmsg.TxnCode/10000 != 39 )
				{
					if ( ( atoi(tmsmsg.Amount) > 0 || atof(tmsmsg.Amount) > 0) && (tmsmsg.MsgType == 200))
					{	
						trace("Limit check entry");
						if (ezmmsdbutil.readEzAgentLimitInfo( &tmsmsg ) < 0 )
						{
							trace("Error in LIMIT CHECK & ACCUM VALIDATION TRANSACTION DECLINED....");
							tmsmsg.MsgType += 10;
							//tmsmsg.RespCode = EZTMS_RC_UnableToProcess;
							//writeToPOS( GetLastSenderChannelName(), &tmsmsg );
						}
					...
					if ( ezmmsdbutil.readAgentSubagentFeeAmount( &tmsmsg ) < 0)
					{
						trace("Error in Reading Fee for Agent / Subagent transaction ...");
						continue;
					}				
				}
				*/
		...
		if( isInBoundTxnMsg() && tmsmsg.MsgType != 400 && tmsmsg.MsgType != 420 )
		{
			...
			*/
			...
			/*
			// Accumulation for AGENCY POS - Mohan
			
			trace(" CHeck .....2. tmsgtype[%04d] , tmsrescode[%d]  tmsmerchat[%s] tmsstore[%s] ",tmsmsg.MsgType, tmsmsg.RespCode, tmsmsg.AcceptorId, tmsmsg.StoreId);
				//if ( tmsmsg.MsgType == 210 && (( isInBoundFinTxnResponse() &&  tmsmsg.RespCode == 0) && (atoi(tmsmsg.Amount) > 0 || atof(tmsmsg.Amount) > 0 )))
				if ( tmsmsg.MsgType == 210 && tmsmsg.RespCode == 0 && (atoi(tmsmsg.Amount) > 0 || atof(tmsmsg.Amount) > 0 ))
				{
					if(ezmmsdbutil.accumulateAgencyTransactionAmount( &tmsmsg ,'1' ) < 0)

					{
						trace("Error in ACCUM LEVEL VALIDATION & UPDATE .....");
						continue;
				
					}
				}
				//if( ( tmsmsg.MsgType == 410 || tmsmsg.MsgType == 410 )&& ( isInBoundFinTxnResponse()&&( atoi(tmsmsg.Amount) > 0 || atof(tmsmsg.Amount) > 0 )))
				if( ( tmsmsg.MsgType == 410 || tmsmsg.MsgType == 430 ) && ( atoi(tmsmsg.Amount) > 0 || atof(tmsmsg.Amount) > 0 ))
				{
					if(ezmmsdbutil.accumulateAgencyTransactionAmount( &tmsmsg ,'2' ) < 0)

                                        {
                                                trace("Error in DECREMENT LEVEL VALIDATION & UPDATE .....");
						continue;
                                        }
				}
			*/
...
void EzAgentTMSDriver :: processTerminalUserRequest( struct EzTxnMsg *tmsmsg )
{
	int retval;
	char TermPINKey_LMK[ 65 ], TermPINKey_TMK[ 65 ], TermPINKey_KCV[ 65 ], TempStr[ 255 ];
	char CatCode[ 17 ], UserName[ 33 ], Password[ MMS_PWD_SIZE + 1 ], hPassword[ MMS_PWD_SIZE + 1 ];
...
{
	EzAgentTMSDriver ezagenttmsdriverobj;
	ezagenttmsdriverobj.initAgentTMSDriver( argv[ 0 ] );
	ezagenttmsdriverobj.serviceRequest();
	return 1;
}
