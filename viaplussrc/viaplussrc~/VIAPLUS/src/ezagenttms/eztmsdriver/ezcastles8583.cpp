	...
	for( int bitno = 1; bitno <= 128; bitno++ )
	{
		...
		switch ( bitno )
		{
			...
			case 64:
				...
				//MAC Code
				InBuf += fmtutilobj.fmt_in( InBuf, &len, BCDTOI, 3, bitno );
				InBuf += fmtutilobj.fmt_in( InBuf, tmsmsg->RsvFld2, ETOA, len, bitno );
				break ;
	...
	for( int bitno = 1; bitno <= 128; bitno++ )
	{
		...
		switch ( bitno )
		{
			...
			case 63:
				...
				len = strlen( PrinterResponseTemplate );
				OutBuf += fmtutilobj.fmt_out( OutBuf, &len, ITOBCD, 3, bitno );
				OutBuf += fmtutilobj.fmt_out( OutBuf, PrinterResponseTemplate, CHAR, len, bitno );
				break ;
