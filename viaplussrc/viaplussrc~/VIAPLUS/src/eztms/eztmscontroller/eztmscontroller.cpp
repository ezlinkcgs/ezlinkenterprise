...
void EzTMSController :: processTerminalInfoDownload( struct EzTxnMsg *tmsmsg )
{
	...
	sprintf( tmsmsg->RsvFld3, "%s%c%s%c%s%c%s%c%s%c%s%c", merchantprofile_ptr.InstId, 0x1c, merchantprofile_ptr.MerchantName, 0x1c, storeprofile_ptr.StoreName, 0x1c, storeprofile_ptr.StoreLocation, 0x1c, storeprofile_ptr.CityName, 0x1c, storeprofile_ptr.CityCode, 0x1c );
...
void EzTMSController :: getNewConfigId( char *ConfigId )
{
	...
	if( ConnectDB() < 0 )
		trace( "Error in Connecting to Switch Database....Cannot Update ConfigId" );
	else
	{
		trace(" machineid[%s]",terminalprofile_rec.MachineId);
		int retval = updateNewConfigId( terminalprofile_rec.MachineId,  ConfigId );
		if( retval == 0 )
			trace( "Update New ConfigId[%s] in EzMMSTerminalProfile...", ConfigId );
		else
