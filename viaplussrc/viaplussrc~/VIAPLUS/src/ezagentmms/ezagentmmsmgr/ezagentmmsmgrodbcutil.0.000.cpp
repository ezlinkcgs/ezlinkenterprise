 ...
 *      Author: mohanraj
 */
#include <ezagentmms/ezagentmmsmgrodbcutil.h>

//int EzMMSMgrODBCUtil :: initEzMMSMgrODBCUtil( const char *TaskName, EzDebugUtil *dptr )
int EzMMSMgrODBCUtil :: initEzMMSMgrODBCUtil( const char *TaskName, EzDebugUtil *dptr, const char *DBGroupName )
{
	this->dptr = dptr;
	if( initODBCUtil( TaskName, dptr , DBGroupName ) < 0 )
	{
...
int EzMMSMgrODBCUtil :: getEzMerchantSchemeCount( int *SqlErr )
{
    int count;
    char SqlStmt[ 1024 ];
    ...
    *SqlErr = 0;
    ...
    sprintf( SqlStmt, "SELECT COUNT(*) FROM EZMMS_MERCHANTSCHEME" );
    getTableRowCount( SqlStmt, &count );
    dptr->trace( "EZMMS_MERCHANTSCHEME - Count[%d]", count);

    return count;
}

int EzMMSMgrODBCUtil :: LoadEzMerchantScheme( struct EzMMSMerchantScheme *merchantscheme_ptr, int *SqlErr )
{
	char InstId[ 13 ];
	char BIN[ 13 ];
	char SchemeName[ 17 ];  
    ...
    *SqlErr = 0;
    ...
    {
		dptr->trace( DEBUG_ERROR, "Error in Allocation Statement Handle in LoadEzMerchantScheme [ %d ]", odbcret );
		return -1;
    }
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	...
    	dptr->trace( DEBUG_ERROR, "Error in Executing SqlStmt in LoadEzMerchantScheme [ %d ]", odbcret );
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(InstId) in EZMMS_MERCHANTSCHEME [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(BIN) in EZMMS_MERCHANTSCHEME [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }

    odbcret = SQLBindCol( hstmt, 3, SQL_C_CHAR, SchemeName, sizeof(SchemeName), &Len );
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(SchemeName) in EZMMS_MERCHANTSCHEME [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }

    odbcret = SQLBindCol( hstmt, 4, SQL_C_CHAR, SchemeDesc, sizeof(SchemeDesc), &Len );
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(SchemeDesc) in EZMMS_MERCHANTSCHEME [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }
...
int EzMMSMgrODBCUtil :: getMMSTxnCodeDescCount(int *SqlErr )
{
    int count;
    char SqlStmt[ 1024 ];
    ...
    *SqlErr = 0;
    memset( SqlStmt, 0, sizeof( SqlStmt ) );
    sprintf( SqlStmt, "SELECT COUNT(*) FROM FROM EZMMS_TXNCODEDESC" );
    getTableRowCount( SqlStmt, &count );
    dptr->trace( "FROM EZMMS_TXNCODEDESC - Count[%d]", count);

    return count;
}

int EzMMSMgrODBCUtil :: LoadMMSTxnCodeDescTable( struct EzMMSTxnCodeDesc *txncodedesc_ptr, int *SqlErr )
{
    char RecordId[ 33 ];
	char InstId[ 13 ];
    char TxnCode[ 7 ];
    ...
    *SqlErr = 0;
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
		dptr->trace( DEBUG_ERROR, "Error in Allocation Statement Handle in LoadMMSTxnCodeDescTable[ %d ]", odbcret );
		return -1;
    }
    ...
    {
    	dptr->trace("SqlStmt<%s>", SqlStmt);
    	dptr->trace( DEBUG_ERROR, "Error in Executing SqlStmt in LoadMMSTxnCodeDescTable [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(RecordId) in EZMMS_TXNCODEDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(InstId) in EZMMS_TXNCODEDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(TxnCode) in EZMMS_TXNCODEDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }

    odbcret = SQLBindCol( hstmt, 4, SQL_C_CHAR, ShortDesc, sizeof(ShortDesc), &Len );
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(ShortDesc) in EZMMS_TXNCODEDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }

    odbcret = SQLBindCol( hstmt, 5, SQL_C_CHAR, LongDesc, sizeof(LongDesc), &Len );
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(LongDesc) in EZMMS_TXNCODEDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }

    odbcret = SQLBindCol( hstmt, 6, SQL_C_CHAR, TxnType, sizeof(TxnType), &Len );
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(TxnType) in EZMMS_TXNCODEDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }
...
int EzMMSMgrODBCUtil :: getMMSTransactionDescCount(int *SqlErr )
{
    int count;
    char SqlStmt[ 1024 ];
    ...
    *SqlErr = 0;
    memset( SqlStmt, 0, sizeof( SqlStmt ) );
    sprintf( SqlStmt, "SELECT COUNT(*) FROM FROM EZMMS_TRANSACTIONDESC" );
    getTableRowCount( SqlStmt, &count );
    dptr->trace( "FROM EZMMS_TRANSACTIONDESC - Count[%d]", count);

    return count;
}
int EzMMSMgrODBCUtil :: LoadMMSTransactionDescTable( struct EzMMSTransactionDesc *transactiondesc_ptr, int *SqlErr )
{
    char RecordId[ 33 ];
	char InstId[ 13 ];
	char DeviceType[ 17 ];
    ...
    *SqlErr = 0;
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
		dptr->trace( DEBUG_ERROR, "Error in Allocation Statement Handle in LoadMMSTransactionDescTable[ %d ]", odbcret );
		return -1;
    }
    ...
    {
    	dptr->trace("SqlStmt<%s>", SqlStmt);
    	dptr->trace( DEBUG_ERROR, "Error in Executing SqlStmt in LoadMMSTransactionDescTable [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(RecordId) in EZMMS_TRANSACTIONDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(InstId) in EZMMS_TRANSACTIONDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }

    odbcret = SQLBindCol( hstmt, 4, SQL_C_CHAR, DeviceType, sizeof(DeviceType), &Len );
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(DeviceType) in EZMMS_TRANSACTIONDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }
    ...
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(TxnCode) in EZMMS_TRANSACTIONDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }

    odbcret = SQLBindCol( hstmt, 6, SQL_C_CHAR, TxnDesc, sizeof(TxnDesc), &Len );
    if( !SQL_SUCCEEDED( odbcret ) )
    {
    	dptr->trace( DEBUG_ERROR, "Error in Binding Column(TxnDesc) in EZMMS_TRANSACTIONDESC [ %d ]", odbcret );
		SQLFreeHandle( SQL_HANDLE_STMT, hstmt );
		return -1;
    }
