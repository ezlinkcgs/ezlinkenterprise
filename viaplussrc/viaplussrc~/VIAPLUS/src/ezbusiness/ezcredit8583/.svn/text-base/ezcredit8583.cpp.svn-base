#include <ezbusiness/ezcredit8583.h>

EzCredit8583 :: EzCredit8583()
{
}
...
void EzCredit8583 :: InitEzCredit8583( const char *TaskName )
{
	...
	trace( "^^^^^^^^^^^EzCredit8583 Started Running^^^^^^^^^^^^^");

	if( InitEzTaskUtil( this ) < 0 )
	{
		trace( "Error in Initializing EzTaskUtil");
		trace( "^^^^^^^^^^^EzCredit8583 Ended Abnormally^^^^^^^^^^^^^");
		closeDebug();
		exit( 0 );
	}
...
int EzCredit8583 :: getUBPToAcct( int TxnCode, char *ToAcct )
{
	EzCfgUtil cfgutil;
...
void EzCredit8583 :: removeLeadingZeros( const char *SrcStr, char *DestStr )
{
	int i = 0;
...
int EzCredit8583 :: ComposeISOMessage( struct EzTxnMsg *txnmsg, char *OutBuf )
{
...
int EzCredit8583 :: DecomposeISOMessage( char *InBuf, struct EzTxnMsg *txnmsg )
{
	unsigned char BitMap[ 33 ];
	int Ind, len;
...
void EzCredit8583 :: ServiceRequest()
{
	...
	while( 1 )
	{
		trace( "EzCredit8583 Waiting for Request to be Serviced" );
...
{
	EzCredit8583 ezcredit8583obj;
	ezcredit8583obj.InitEzCredit8583( argv[ 0 ] );
	ezcredit8583obj.ServiceRequest();
	return 1;
}
