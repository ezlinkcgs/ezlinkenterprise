#include <ezbusiness/ezcirrus8583.h>

EzCirrus8583 :: EzCirrus8583()
{
}
...
void EzCirrus8583 :: InitEzCirrus8583( const char *TaskName )
{
	...
	trace( "^^^^^^^^^^^EzCirrus8583 Started Running^^^^^^^^^^^^^");

	if( InitEzTaskUtil( this ) < 0 )
	{
		trace( "Error in Initializing EzTaskUtil");
		trace( "^^^^^^^^^^^EzCirrus8583 Ended Abnormally^^^^^^^^^^^^^");
		closeDebug();
		exit( 0 );
	}
	...
	sprintf( BitMapFile, "%s/%s/%s/%s", getenv( "EZDIR_HOME" ), "cfg", "iso8583", "EzCirrus8583BitMap.cfg" );
...
int EzCirrus8583 :: ComposeISOMessage( struct EzTxnMsg *txnmsg, char *OutBuf )
{
...
int EzCirrus8583 :: DecomposeISOMessage( char *InBuf, struct EzTxnMsg *txnmsg )
{
	unsigned char BitMap[ 33 ];
	int Ind, len;
...
//		hex_dump( (unsigned char *)BitMap, 16 );
	...
	if( txnmsg->MsgType == 420 )
	{
		...
		strcpy( txnmsg->OrgChannel, "EzCirrus8583");
		strcpy( txnmsg->OrgPort, "Cirrus");
	}

	strcpy( txnmsg->SrcChannel, "EzCirrus8583");
	strcpy( txnmsg->SrcPort, "Cirrus");


	if( txnmsg->MsgType == 800 )
		strcpy( txnmsg->NetworkId, "Cirrus");
		...
		switch ( bitno )
		{
			...
			case 32://Acquiring Institution Identification Code
				...
				if( txnmsg->MsgType == 200 || txnmsg->MsgType == 420 || txnmsg->MsgType == 421 )
					strcpy( txnmsg->AcquirerInst, "Cirrus" );
				break ;
...
void EzCirrus8583 :: removeLeadingZeros( const char *SrcStr, char *DestStr )
{
	int i = 0;
...
void EzCirrus8583 :: ServiceRequest()
{
	...
	while( 1 )
	{
		trace( "EzCirrus8583 Waiting for Request to be Serviced" );
...
{
	EzCirrus8583 EzCirrus8583obj;
	EzCirrus8583obj.InitEzCirrus8583( argv[ 0 ] );
	EzCirrus8583obj.ServiceRequest();
	return 1;
}
