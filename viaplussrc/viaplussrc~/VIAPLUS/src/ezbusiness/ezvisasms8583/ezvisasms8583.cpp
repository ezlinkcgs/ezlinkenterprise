#include <ezbusiness/ezvisasms8583.h>

EzVISASMS8583 :: EzVISASMS8583()
{
}
...
void EzVISASMS8583 :: InitEzVISASMS8583( const char *TaskName )
{
	...
	trace( "^^^^^^^^^^^EzVISASMS8583 Started Running^^^^^^^^^^^^^");

	if( InitEzTaskUtil( this ) < 0 )
	{
		trace( "Error in Initializing EzTaskUtil");
		trace( "^^^^^^^^^^^EzVISASMS8583 Ended Abnormally^^^^^^^^^^^^^");
		closeDebug();
		exit( 0 );
	}
	...
	sprintf( BitMapFile, "%s/%s/%s/%s", getenv( "EZDIR_HOME" ), "cfg", "iso8583", "EzVISASMS8583BitMap.cfg" );
...
int EzVISASMS8583 :: ComposeISOMessage( struct EzTxnMsg *txnmsg, char *OutBuf )
{
...
int EzVISASMS8583 :: DecomposeISOMessage( char *InBuf, struct EzTxnMsg *txnmsg )
{
	unsigned char BitMap[ 33 ];
	int Ind, len;
...
//		hex_dump( (unsigned char *)BitMap, 16 );
	...
	if( txnmsg->MsgType == 420 )
	{
		...
		strcpy( txnmsg->OrgChannel, "EzVISASMS8583");
		strcpy( txnmsg->OrgPort, "VISASMS");
	}

	strcpy( txnmsg->SrcChannel, "EzVISASMS8583");
	strcpy( txnmsg->SrcPort, "VISASMS");


	if( txnmsg->MsgType == 800 )
		strcpy( txnmsg->NetworkId, "VISASMS");
		...
		switch ( bitno )
		{
			...
			case 32://Acquiring Institution Identification Code
				...
				if( txnmsg->MsgType == 200 || txnmsg->MsgType == 420 || txnmsg->MsgType == 421 )
					strcpy( txnmsg->AcquirerInst, "VISASMS" );
				break ;
...
void EzVISASMS8583 :: removeLeadingZeros( const char *SrcStr, char *DestStr )
{
	int i = 0;
...
void EzVISASMS8583 :: ServiceRequest()
{
	...
	while( 1 )
	{
		trace( "EzVISASMS8583 Waiting for Request to be Serviced" );
...
{
	EzVISASMS8583 EzVISASMS8583obj;
	EzVISASMS8583obj.InitEzVISASMS8583( argv[ 0 ] );
	EzVISASMS8583obj.ServiceRequest();
	return 1;
}
