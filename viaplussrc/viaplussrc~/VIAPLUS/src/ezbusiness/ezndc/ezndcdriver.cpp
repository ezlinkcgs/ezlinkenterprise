...
void EzNDC :: ServiceRequest()
{
	...
	while( 1 )
	{
		...
		trace( DEBUG_SECURED, "Message of size [%d] Received from Channel[%s]", BufSize, GetLastSenderChannelName() );
	        hex_dump( ( unsigned char * )ReadBuf, ( unsigned int )strlen( ReadBuf ) );
...
int EzNDC :: DiagnizeAndServeContReq( struct EzAtmController ContReq , char *AtmId )
{
	...
	{
		trace( DEBUG_NORMAL, "Received Operational Command For ATM[ %s ]", AtmId );
	...
	{
		trace( DEBUG_ERROR, "Invalid Message Type [ %d ] for ATM [ %s ]", ContReq.MsgType, AtmId );
		return -1;
	}
	return 1;
}
        ...
        case 'F':
			...
			switch( *AtmReq )
			{
				...
				case '2':
					...
					//By Mohan - For Auto get supply counters
					memset( &AtmCont, 0, sizeof( struct EzAtmController ) );
					AtmCont.MsgType = EZ_ATM_COMMAND_REQ;
	...
	switch ( *AtmReq )
	{
		case 'A':
			...
			case 'B':
				...
				trace("CurrentStatus[%c]", atmbuf.CurrentState);
			...
			case 'D':
				...
				switch( *AtmReq )
				{
					...
					case '1':
						...
						/* Added for Cash Before card out */
						devmsg.RespCode = EZLINK_RC_UnableDispense;
						AtmReq++;
						trace( "Marked this transaction as UnableDispense txn RespCode[%d]", devmsg.RespCode );
						break;
					case '2':
				...
				switch( *AtmReq )
				{
					...
					default:
						...
						break;
                                ...
                                if( atmbuf.CurrentState == TRANSACTIONRESPONSE )
                                {
                                        if( ( devmsg.TxnCode/10000 == 1 ) && devmsg.RespCode != 0 )
                                        {
						trace( "Generating Card Intake Reversal");
                                                ...
                                                trace( "Routing to BuildAndPostFuncMsg");
			...
			case 'E':
				...
				if( atmbuf.CurrentState == TRANSACTIONRESPONSE )
				{
					...
					if( devmsg.RespCode != EZLINK_RC_DispencedFully )
					{
						...
						devmsg.MsgType = EZLINK_TXN_TxnResponse;
						/* commented for NCR by Hari on 29/1/2011 */
						if( BuildAndPostFuncMsg( devmsg ) < 0 )
						{
							trace( DEBUG_ERROR, "Failed in BuildAndPostFuncMsg");
							return -1;
...
int EzNDC :: ServeConsumerRequests( char * AtmReq, int Size , char *AtmId )
{
	...
	EzCfgUtil cfg;
	EzDateTimeUtil dtu;
	char LastDispenceLogicBuf[ DISP + 1 ];
	char LTSData[ LTS_DATA_SIZE + 1 ];

	struct EzDevMsg ConsumerReq, OldDevMsg, LastDevMsg ;
	...
	memset( LastDispenceLogicBuf, 0, sizeof( LastDispenceLogicBuf ) );
	strcpy( LastDispenceLogicBuf, atmbuf.DispenceLogicBuf );
	...
	if( OffSetPtr != NULL )
	{
		...
		if ( !isdigit( ConsumerReq.Track2[ strlen( ConsumerReq.Track2 ) - 1 ] ) )
			ConsumerReq.Track2[ strlen( ConsumerReq.Track2 ) - 1 ] = '\0';
		...
		else
		{
			...
			if( atoi( TxnReqFlag ) == NewTxn )
			{
				...
				if( atminfo.TraceNum != 9999 )
				{
					...
					ConsumerReq.PosCapCode = atminfo.TraceNum;
					atminfo.TraceNum = atminfo.TraceNum + 1;
				}
				else
				{
					ConsumerReq.PosCapCode = 1;
					atminfo.TraceNum = 1;
				}

				trace( DEBUG_TEST, "Transaction TraceNo[ %d ]", ConsumerReq.PosCapCode );
	...
	{
		if( strlen( PinBuf )== 0 && atmbuf.CurrentState != INTERACTIVE )
		{       
			trace("PINBLOCK CHECKING");
	...
	//Track1
	memset( temp, 0, sizeof( temp ) );
	OffSetPtr = ( char * ) memchr( AtmReq, DEVICE_FS, 120 );
	if( OffSetPtr != NULL )
	{
		memcpy( temp, AtmReq, ( OffSetPtr - AtmReq ) );
	...
	{
		strcpy( LTSData, AtmReq );
		trace( DEBUG_TEST, "LTS[%s]", LTSData );
	}
	
	//trace( "Checking LTS Parser ");
        /*if( CheckLTS_TVS( &OldDevMsg, LastDispenceLogicBuf, LTSData ) != -1 )
        {
                if( PostTxnMsg( &OldDevMsg, AtmId ) < 0 )
                {
                        trace( DEBUG_ERROR, "Failed in PostTxnMsg" );
                        return -1;
                }
	...
	trace("CurrentState[ %c ]",  atmbuf.CurrentState );
...
int EzNDC :: BuildAndPostWriteCmd( struct EzAtmController ContReq)
{
	...
	switch( ContReq.DownloadType )
	{
		...
		case DOWNLOAD_MISC_E:
			...
			sprintf( AtmBuf, "30%c001%c999%c1A%c%c15001", 0x1c, 0x1c, 0x1c, 0x1c,0x1c );
			break;
		...
		default:
			trace( "Invalid Download<%d>", ContReq.DownloadType);
			break;
	}
...
int EzNDC :: BuildAndPostFuncMsg( struct EzDevMsg DevMsg )
{
	...
	sprintf( temp, "%04d", DevMsg.PosCapCode );
			...
			case 1 :// Cash Withdrawal
				if ( DevMsg.RespCode == EZLINK_RC_Approved )
				{
					if( atmbuf.PrinterFlag == '1' )
					{
						...
						{
					//commented for card before cash by vaithi
						//	strcat( FunctionMsg, "8" );
							strcat( FunctionMsg, "A" );
				...
				else
				{
					...
					if( DevMsg.TxnCode/10000 == 20 )
					{
						...
						else
						{
							...
							strcat( FunctionMsg, "\x1d");
							strcat( FunctionMsg, "4" );//Print in Envelope Printer
							strcat( FunctionMsg, PrinterData );
						}
					}
					strcat( FunctionMsg, "\x1c");
...
int EzNDC :: PostTxnMsg( struct EzDevMsg *DevMsg, char *AtmId )
{
	...
	trace( DEBUG_TXN, "Writing MsgType[ %d ] TxnCode[ %d ] RespCode[ %d ] TraceNo[ %d ] Chn[ %s ] Amount[ %s ] DispAmount[ %s ] to Channel[%s]", eztxnmsg.MsgType, eztxnmsg.TxnCode, eztxnmsg.RespCode, eztxnmsg.TraceNo, eztxnmsg.chn, eztxnmsg.Amount, eztxnmsg.DispAmount, TRANSACTIONCHANNEL );
