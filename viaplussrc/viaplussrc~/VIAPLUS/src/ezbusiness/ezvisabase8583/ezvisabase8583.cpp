#include <ezbusiness/ezvisabase8583.h>

EzVISABASE8583 :: EzVISABASE8583()
{
}
...
void EzVISABASE8583 :: InitEzVISABASE8583( const char *TaskName )
{
	...
	trace( "^^^^^^^^^^^EzVISABASE8583 Started Running^^^^^^^^^^^^^");

	if( InitEzTaskUtil( this ) < 0 )
	{
		trace( "Error in Initializing EzTaskUtil");
		trace( "^^^^^^^^^^^EzVISABASE8583 Ended Abnormally^^^^^^^^^^^^^");
		closeDebug();
		exit( 0 );
	}
	...
	sprintf( BitMapFile, "%s/%s/%s/%s", getenv( "EZDIR_HOME" ), "cfg", "iso8583", "EzVISABASE8583BitMap.cfg" );
...
int EzVISABASE8583 :: ComposeISOMessage( struct EzTxnMsg *txnmsg, char *OutBuf )
{
...
int EzVISABASE8583 :: DecomposeISOMessage( char *InBuf, struct EzTxnMsg *txnmsg )
{
	unsigned char BitMap[ 33 ];
	int Ind, len;
...
//		hex_dump( (unsigned char *)BitMap, 16 );
	...
	if( txnmsg->MsgType == 420 )
	{
		...
		strcpy( txnmsg->OrgChannel, "EzVISABASE8583");
		strcpy( txnmsg->OrgPort, "VISABASE");
	}

	strcpy( txnmsg->SrcChannel, "EzVISABASE8583");
	strcpy( txnmsg->SrcPort, "VISABASE");


	if( txnmsg->MsgType == 800 )
		strcpy( txnmsg->NetworkId, "VISABASE");
		...
		switch ( bitno )
		{
			...
			case 32://Acquiring Institution Identification Code
				...
				if( txnmsg->MsgType == 200 || txnmsg->MsgType == 420 || txnmsg->MsgType == 421 )
					strcpy( txnmsg->AcquirerInst, "VISABASE" );
				break ;
...
void EzVISABASE8583 :: removeLeadingZeros( const char *SrcStr, char *DestStr )
{
	int i = 0;
...
void EzVISABASE8583 :: ServiceRequest()
{
	...
	while( 1 )
	{
		trace( "EzVISABASE8583 Waiting for Request to be Serviced" );
...
{
	EzVISABASE8583 EzVISABASE8583obj;
	EzVISABASE8583obj.InitEzVISABASE8583( argv[ 0 ] );
	EzVISABASE8583obj.ServiceRequest();
	return 1;
}
