...
void EzHost8583 :: InitEzHost8583( const char *TaskName )
{
	...
	sprintf( BitMapFile, "%s/%s/%s/%s", getenv( "EZDIR_HOME" ), "cfg", "iso8583", "EzHost8583BitMap.cfg" );
...
int EzHost8583 :: ComposeISOMessage( struct EzTxnMsg *txnmsg, char *OutBuf )
{
	...
	if( txnmsg->MsgType == 800 )
	{
		...
		strcpy( txnmsg->NetworkId, "502177" );
	}
	else
	{
		memset( txnmsg->AcqNetworkId, 0, sizeof( txnmsg->AcqNetworkId ) );
		strcpy( txnmsg->AcqNetworkId, "502177" );
	}
		...
		switch ( bitno )
		{
			...
			case 32:
				...
				else
				{
					...
					memset( txnmsg->AcqNetworkId, 0, sizeof( txnmsg->AcqNetworkId ) );
					strcpy( txnmsg->AcqNetworkId, "hpscb" );
				}
				break ;
			case 37:
