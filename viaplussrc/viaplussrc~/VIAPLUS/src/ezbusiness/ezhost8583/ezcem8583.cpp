...
int EzHost8583 :: GetPOSFee( char *Merch_ID, char *Fee )
{
        EzCfgUtil cfgutil;
        char cfgpath[ CFG_PATH_SIZE + 1 ], cfgval[ CFG_VAL_SIZE + 1 ], SearchId[ CFG_VAL_SIZE + 1 ];

        memset( cfgpath, 0, sizeof( cfgpath ) );
        sprintf( cfgpath, "%s/%s/%s", getenv( "EZDIR_HOME" ), "cfg", "Fee.conf" );
        ...
        memset( SearchId, 0, sizeof( SearchId ) );
        sprintf( SearchId, "%s#%s", "FEE", Merch_ID );
        trace( DEBUG_NORMAL, "SearchId [%s]", SearchId );

        memset( cfgval, 0, sizeof( cfgval ) );
        if( cfgutil.cfg_search( SearchId, cfgval, 2 ) < 0 )
        {
                trace( DEBUG_ERROR, "Error in Searching FEE#MERCH_ID from file[ %s ]", cfgpath );
                cfgutil.cfg_close();
                return -1;
        }
        else
...
int EzHost8583 :: ComposeISOMessage( struct EzTxnMsg *txnmsg, char *OutBuf )
{
	...
	*/
	...
	if( !strcmp( txnmsg->DeviceType, "POS" ) && txnmsg->TxnCode/10000 == 61)
	{
		SetBitOn( BitMap, 42 );
		SetBitOn( BitMap, 28 );
	}
	...
	for( int bitno = 1; bitno <= 128; bitno++ )
	{
		...
		switch ( bitno )
		{
			...
			case 28:
       			 char cfgpath[151],SearchForFee[30],SearchValue[13],SendFee[13];
       			 int returnlen,cmp;
       			 memset( cfgpath, 0 , sizeof( cfgpath ) );
       			 memset( SearchForFee, 0 , sizeof( SearchForFee ) );
        		 memset( SearchValue, 0 , sizeof( SearchValue ) );
      			 memset( SendFee, 0 , sizeof( SendFee ) );

                            if( !strcmp( txnmsg->DeviceType, "POS" ) )  /* Fee For POS Txn's */
                                strcpy( SearchForFee, txnmsg->AcceptorId );
                            else
                                strcpy( SearchForFee,"MinistatementFee");

				if( GetPOSFee( SearchForFee, SearchValue ) < 0 )
				{
					trace( DEBUG_ERROR, "Error in Loading Merch_ID");
					return -1;
				}

                               if ( strlen(SearchValue) == 10)
			       { 
			           sprintf(SendFee,"%s00",SearchValue);
				   trace("Fee Amount is [%s]",SendFee ); 
			       }
               		       else 
			       {
                                  strcpy(SendFee,"000000000000");
                                  trace("Bad Fee config!.. Fee length must be 10... Fee Disabled!..Sending Fee[%s]",SendFee);
			       }
                          returnlen=0;
