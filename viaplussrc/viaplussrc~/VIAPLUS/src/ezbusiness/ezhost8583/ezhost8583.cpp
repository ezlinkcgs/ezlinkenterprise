...
void formatAmt( const char *Amount, char *OutStr, int Len )
{
char formatStr[ 50 ];
...
/*			case 4:
			...
			case 4:
				...
				OutBuf += fmt_out( OutBuf, TempStr, FMT_CHAR, 12, bitno );
				break ;
	...
	for( int bitno = 1; bitno <= 128; bitno++ )
	{
		...
		switch ( bitno )
		{
			...
			case 4:
				...
				if( ( txnmsg->TxnCode/10000 ) == 1 )
				{
					Amount = atoi( TempAmount ) / 1000;
					sprintf( txnmsg->Amount, "%d.000", Amount );
				}
				else
				{
					memset( temp, 0, sizeof( temp ) );
					memset( decimal, 0, sizeof( decimal ) );
					strncpy( temp, TempAmount, strlen( TempAmount )-3 );
					strncpy( decimal, TempAmount + strlen( TempAmount ) - 3, 3 );
					memset( TempAmount, 0, sizeof( TempAmount ) );
