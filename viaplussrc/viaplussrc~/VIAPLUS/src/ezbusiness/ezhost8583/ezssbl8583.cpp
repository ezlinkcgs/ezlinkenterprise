...
/*			case 102:
                                char TempAcct[32];
                                if( txnmsg->MsgType == 200 || txnmsg->MsgType == 220 || ( strlen( txnmsg->FromAccount ) <= 7 ) )
                                {
                                        memset( TempAcct, 0, sizeof( TempAcct ));
                                        strcpy( TempAcct, txnmsg->IssBranchCode );
                                        strcat( TempAcct, txnmsg->FromAcctSubType);
                                        strcat( TempAcct, txnmsg->FromAccount );
                                        trace( DEBUG_NORMAL, "List Acct[ %s ]\n", TempAcct);
                                        len = strlen( TempAcct );
                                        OutBuf += fmt_out( OutBuf, &len, ITOA, 2, bitno );
