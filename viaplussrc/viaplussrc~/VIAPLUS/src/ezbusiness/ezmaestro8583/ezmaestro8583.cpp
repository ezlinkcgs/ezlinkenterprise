#include <ezbusiness/ezmaestro8583.h>

EzMaestro8583 :: EzMaestro8583()
{
}
...
void EzMaestro8583 :: InitEzMaestro8583( const char *TaskName )
{
	...
	trace( "^^^^^^^^^^^EzMaestro8583 Started Running^^^^^^^^^^^^^");

	if( InitEzTaskUtil( this ) < 0 )
	{
		trace( "Error in Initializing EzTaskUtil");
		trace( "^^^^^^^^^^^EzMaestro8583 Ended Abnormally^^^^^^^^^^^^^");
		closeDebug();
		exit( 0 );
	}
	...
	sprintf( BitMapFile, "%s/%s/%s/%s", getenv( "EZDIR_HOME" ), "cfg", "iso8583", "EzMaestro8583BitMap.cfg" );
...
int EzMaestro8583 :: ComposeISOMessage( struct EzTxnMsg *txnmsg, char *OutBuf )
{
...
int EzMaestro8583 :: DecomposeISOMessage( char *InBuf, struct EzTxnMsg *txnmsg )
{
	unsigned char BitMap[ 33 ];
	int Ind, len;
...
//		hex_dump( (unsigned char *)BitMap, 16 );
	...
	if( txnmsg->MsgType == 420 )
	{
		...
		strcpy( txnmsg->OrgChannel, "EzMaestro8583");
		strcpy( txnmsg->OrgPort, "NFS");
	}

	strcpy( txnmsg->SrcChannel, "EzMaestro8583");
...
void EzMaestro8583 :: removeLeadingZeros( const char *SrcStr, char *DestStr )
{
	int i = 0;
...
void EzMaestro8583 :: ServiceRequest()
{
	...
	while( 1 )
	{
		trace( "EzMaestro8583 Waiting for Request to be Serviced" );
...
{
	EzMaestro8583 EzMaestro8583obj;
	EzMaestro8583obj.InitEzMaestro8583( argv[ 0 ] );
	EzMaestro8583obj.ServiceRequest();
	return 1;
}
