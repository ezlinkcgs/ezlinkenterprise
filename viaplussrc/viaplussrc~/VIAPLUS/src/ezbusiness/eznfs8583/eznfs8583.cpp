#include <ezbusiness/eznfs8583.h>
...
void EzNFS8583 :: InitEzNFS8583( const char *TaskName )
{
	...
	trace( "^^^^^^^^^^^EzNFS8583 Started Running^^^^^^^^^^^^^");

	if( InitEzTaskUtil( this ) < 0 )
	{
		trace( "Error in Initializing EzTaskUtil");
		trace( "^^^^^^^^^^^EzNFS8583 Ended Abnormally^^^^^^^^^^^^^");
		closeDebug();
		exit( 0 );
	}
	...
	sprintf( BitMapFile, "%s/%s/%s/%s", getenv( "EZDIR_HOME" ), "cfg", "iso8583", "EzNFS8583BitMap.cfg" );
...
int EzNFS8583 :: ComposeISOMessage( struct EzTxnMsg *txnmsg, char *OutBuf )
{
	char *BufOffset, StmtBuf[ STMT_BUF_SIZE +1 ], Stmt[ 512 ];
	unsigned char BitMap[ 33 ];
	int len, date = 0,utcDate = 0,utcTime = 0;
	char *month, *day, *year;
	BufOffset = OutBuf;
	char ComTempDate[ 7 ],TempAmount[13],TempDate[ 7 ],TempBuf[ 16 ],CheckDate[ 5 ], temp[ 50 ],temptxn[7], TempTime[ 7 ];
	...
	//if( strlen( txnmsg->FromAccount ) == 0 )
        ...
        if( txnmsg->MsgType == 430 && atoi( txnmsg->DispAmount ) != 0 )
        {
             SetBitOn( BitMap, 95 );
        }
	...
	for( int bitno = 1; bitno <= 128; bitno++ )
	{
		...
		switch ( bitno )
		{
			...
			case 3://Processing Code
				...
				else if( txnmsg->TxnCode/10000 == 94 || ( txnmsg->TxnCode/10000 == 38 ) )
					strcpy( temptxn, "900000" );
			...
			*/
			...
			case 42:////Card Acceptor Identification Code
				...
				//	trace("Acceptor identification code is [%s]",txnmsg->RsvFld4);
					OutBuf += fmt_out( OutBuf, txnmsg->RsvFld4, CHAR, 15, bitno );
				break ;

			case 43://Card Acceptor Name/Location
				char *temp,*temp1;
				temp = strtok_r( txnmsg->TerminalLocation, "~",&temp1 );
//				OutBuf += fmt_out( OutBuf, txnmsg->TerminalLocation, CHAR, 40, bitno );
				OutBuf += fmt_out( OutBuf, temp, CHAR, 40, bitno );
				break;

			case 48://Additional Data DKE
                                char AdditionalData[ 151 ], Tag[ 10 ], TagLen[ 20 ], Value[ 151 ];
				...
				sprintf( TagLen, "%03d", strlen( Value ) );
				sprintf( AdditionalData, "%s%s%s", Tag, TagLen, Value );
				len = strlen( AdditionalData );	
                                OutBuf += fmt_out( OutBuf, &len, ITOA, 3, bitno );
                                OutBuf += fmt_out( OutBuf, AdditionalData, CHAR, len, bitno );
			...
			case 57://EMV System related data
				break;
                        case 61:
                                char posdatacode[ 42 ];
				int len;
                                memset( posdatacode, 0, sizeof( posdatacode ) );
				
//                                strcpy( posdatacode,"12121222003100041511000000000000000000000" );
				strcpy( posdatacode,"121212220031000" );
				strcat( posdatacode, temp1 );
				strcat( posdatacode,"00000000000000000000");
				len = strlen( posdatacode );
				OutBuf += fmt_out( OutBuf, &len, ITOA, 3, bitno );
                                OutBuf += fmt_out( OutBuf, posdatacode, CHAR, 41, bitno );
                                break;
			...
			case 95://Actual Transaction Amount
				...
				if( txnmsg->MsgType == 430 && atoi( txnmsg->DispAmount ) != 0 )
				{
					OutBuf += fmt_out( OutBuf, txnmsg->RsvFld3,  CHAR, strlen( txnmsg->RsvFld3 ), bitno );
					//SetBitOn( BitMap, 95 );
				}
				else
				{
			...
			case 120:
				...
				if( txnmsg->MsgType == 200 )
				{
					if( txnmsg->TxnCode/10000 == 38 )
						strcpy( Stmt, "00100207002003GDN" );
					else if( txnmsg->TxnCode/10000 == 94 )
					{
						strcpy( Stmt, "00100208002003GDN004016" );
						strcat( Stmt, txnmsg->NewPINBlock );
					}
				...
				else
				{
					if ( txnmsg->TxnCode/10000 == 38 )
					{ 
						strcpy( Stmt, "00100207002003GDN00500210006350" );
						strcat( Stmt, txnmsg->RsvFld1 );
					}
					else if( txnmsg->TxnCode/10000 == 94 )
					{
						strcpy( Stmt, "00100208002003GDN" );
					}
				}
				len = strlen( Stmt );
				OutBuf += fmt_out( OutBuf, &len, ITOA, 3, bitno );
				OutBuf += fmt_out( OutBuf, Stmt, CHAR, len, bitno );
				break;
...
int EzNFS8583 :: DecomposeISOMessage( char *InBuf, struct EzTxnMsg *txnmsg )
{
	unsigned char BitMap[ 33 ];
	int Ind, len;
	char StmtBuf[ STMT_BUF_SIZE +1 ];
	char TempAmount[15], temp[50], decimal[3], temptxn[7], tag[ 5 ], value[ 512 ];
	char ComTempDate[ 7 ], TempDate[ 7 ], CheckDate[ 5 ];
	char *month, *day, *year, *CurrentYear;
	int Amount, ExtRespCode, ExtNetCode, ExtMsgType, date, length;
	char TempTranDate[ 20 ];
	char TempBuffer[ 512 ];
	...
	if( txnmsg->MsgType == 420 )
	{
		...
		strcpy( txnmsg->OrgChannel, "EzNFS8583");
		strcpy( txnmsg->OrgPort, "NFS");
	}

	strcpy( txnmsg->SrcChannel, "EzNFS8583");
	strcpy( txnmsg->SrcPort, "NFS");


	if( txnmsg->MsgType == 800 )
		strcpy( txnmsg->NetworkId, "NFS");
	...
	for( int bitno = 1; bitno <= 128; bitno++ )
	{
		...
		switch ( bitno )
		{
			...
			case 3://Processing Code
				...
				else if( txnmsg->TxnCode/10000 == 90 )
				{
					memset( temp, 0, sizeof( temp ) );
					sprintf( temp, "%06d", txnmsg->TxnCode );
					memset( temptxn, 0, sizeof( temptxn ) );
					if ( atoi( temp + 2 )/100 == 0  )
					{
						if ( txnmsg->MsgType == 200 )
							strcpy( temptxn, "940006" );
						else
							strcpy( temptxn, "940003" );	
					}
					else
					{
						strcpy( temptxn, "38" );
			...
			case 26://POS PIN Capture Code
				memset( temp, 0, sizeof( temp ) );
				InBuf += fmt_in( InBuf, temp, CHAR, 2, bitno );
				break ;
			...
			case 32://Acquiring Institution Identification Code
				...
				if( txnmsg->MsgType == 200 || txnmsg->MsgType == 420 || txnmsg->MsgType == 421 )
					strcpy( txnmsg->AcquirerInst, "BANKC" );
					//strcpy( txnmsg->AcquirerInst, "NFS" );
				break ;
			...
			case 41://Card Acceptor Terminal Identification
				...
				//InBuf += fmt_in( InBuf, txnmsg->Termina lId, CHAR, 8, bitno );
				break ;

			case 42://Card Acceptor Identification Code
				InBuf += fmt_in( InBuf, txnmsg->RsvFld4, CHAR, 15, bitno );
				break ;
			...
			case 102://Account Identification 1
				...
				break;
			case 120: //Ministatement and Pin Change 
				memset( temp, 0, sizeof( temp ) );
				InBuf += fmt_in( InBuf, &len, ATOI, 3, bitno );
				...
				while ( strlen( InBuf ) > 0 )
				{
					memset( tag, 0, sizeof( tag ) );
					memset( value, 0, sizeof( value ) );
					InBuf += fmt_in( InBuf, tag, CHAR, 3, bitno ); //Transaction type tag
					InBuf += fmt_in( InBuf, &length, ATOI, 3, bitno ); //Length of Ttype
					InBuf += fmt_in( InBuf, value, CHAR, length, bitno ); //Data
					
					if( !strcmp( tag, "001" ) )
					{
						if( !strcmp( value, "07" ) )
							txnmsg->TxnCode = 380000;
						else if( !strcmp( value, "36" ) )
							txnmsg->TxnCode = 910000;
						else if( !strcmp( value, "37" ) )
							txnmsg->TxnCode = 920000;
						else if( !strcmp( value, "TC" ) )
							txnmsg->TxnCode = 410000;
						else if( !strcmp( value, "UR" ) )
							txnmsg->TxnCode = 930000;
						else 
						{
							if( txnmsg->MsgType == 200 )
								txnmsg->TxnCode = 940006;
							else
								txnmsg->TxnCode = 940003;
						}
					}		
					if ( !strcmp( tag, "004" ) )
					{
						strcpy( txnmsg->NewPINBlock, value );
						break;
					}
					if( !strcmp( tag, "006" ) )
					{
						strcpy( txnmsg->RsvFld1, value );
						break;
					}	
				}
...
void EzNFS8583 :: removeLeadingZeros( const char *SrcStr, char *DestStr )
{
	int i = 0;
...
void EzNFS8583 :: ServiceRequest()
{
	struct EzTxnMsg txnmsg;
	int MsgSize;
	char ISOMessage[ 4600 + 1 ];
	char MsgBuffer[ 4600 + 1 ];
	struct EzEchoTable echotable_rec;
	while( 1 )
	{
		trace( "EzNFS8583 Waiting for Request to be Serviced" );
		...
		if( IsPortMessage() )
		{
			...
			if( txnmsg.MsgType == 200 && ( txnmsg.TxnCode/10000 ) == 1 )
			{
				trace( "Checking Amount[%d] is valid or not...", atoi( txnmsg.Amount ) );

				if ( ( atoi( txnmsg.Amount ) % 50 ) != 0 )
				{
					txnmsg.MsgType = 210;
					txnmsg.RespCode = 13;
					trace(" Received Invalid Amount From NFS, Sending Invalid Amount Respcode" );
...
{
	EzNFS8583 EzNFS8583obj;
	EzNFS8583obj.InitEzNFS8583( argv[ 0 ] );
	EzNFS8583obj.ServiceRequest();
	return 1;
}
